package com.nazpowerchat.services;
...
import com.nazpowerchat.activities.ChatActivity;
...
public class FirebaseChatService extends Service {
    ...
    public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID_MAIN, "FansTips chat service", NotificationManager.IMPORTANCE_MIN);
            channel.setSound(null, null);
            ((NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE)).createNotificationChannel(channel);
            Notification notification = new NotificationCompat.Builder(this, CHANNEL_ID_GROUP)
                    .setSmallIcon(R.drawable.noti_icon)
                    .setContentTitle("FansTips")
...
//            startService(new Intent(this, this.getClass()));
    ...
    private void notifyNewGroup(Group group) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID_GROUP, "FansTips new group notification", NotificationManager.IMPORTANCE_DEFAULT);
    ...
    private void saveMessage(Message message) {
        ...
        if (Helper.CHAT_NOTIFY && !message.getSenderId().equals(myId) && !helper.isUserMute(message.getSenderId()) && (Helper.CURRENT_CHAT_ID == null || !Helper.CURRENT_CHAT_ID.equals(userOrGroupId))) {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                NotificationChannel channel = new NotificationChannel(channelId, "FansTips new message notification", NotificationManager.IMPORTANCE_DEFAULT);
                notificationManager.createNotificationChannel(channel);
